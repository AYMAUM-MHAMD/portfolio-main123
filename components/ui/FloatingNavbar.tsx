...
export const FloatingNav = ({
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const scrollPosition = window.scrollY + 200; // Adjust offset for Navbar
      ...
      sections.forEach((section, idx) => {
        if (section) {
          ...
          if (
            ...
            setActiveSection(navItems[idx].link);
          }
        }
      });
    };
      ...
      >
        {navItems.map((navItem, idx) => (
          <Link
            key={`link=${idx}`}
            href={navItem.link}
            scroll={false} // مهم! منع السلوك الافتراضي
            onClick={() => {
              ...
              if (target) {
                window.scrollTo({
                  top: target.offsetTop - 140, // مراعاة ارتفاع Navbar
                  behavior: "smooth",
                });
              }
            }}
